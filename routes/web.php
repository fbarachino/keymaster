...
Route::middleware(['auth'])->group(function ()
{
    ...
    // -----------------------------------------------------
        Route::middleware(EnsureTenant::class)->prefix('tenant')->name('tenant.')->group(function () {
            ...
            //Route::get('/tenant/payments', [TenantPaymentController::class, 'index'])->name('tenant.payments.index');
            Route::get('/payments/{payment}/pdf', [TenantPaymentController::class, 'downloadPdf'])->name('tenant.payments.pdf'); });
            ...
            // Signature Leases
            Route::get('/leases/{lease}/sign', [\App\Http\Controllers\Web\Tenant\TenantLeaseSignController::class, 'show']) ->name('leases.sign.show');
            Route::post('/leases/{lease}/sign', [\App\Http\Controllers\Web\Tenant\TenantLeaseSignController::class, 'sign']) ->name('leases.sign.perform');
            Route::get('/archive', [TenantArchiveController::class, 'index'])->name('archive.index');
            Route::get('/archive/monthly/{period}', [TenantArchiveController::class, 'downloadMonthly'])->name('archive.monthly.pdf');
            Route::get('/archive/yearly/{year}', [TenantArchiveController::class, 'downloadYearly'])->name('archive.yearly.pdf');
    ...
    // -----------------------------------------------------
        Route::middleware(EnsureLandlord::class)->prefix('landlord')->name('landlord.')->group(function ()
        {
            ...
            Route::get('/leases/{lease}/monthly/{period}/pdf', [LandlordLeaseController::class, 'downloadMonthlyPdf'] )->name('lease.monthly.pdf');
            Route::get('/leases/{lease}/yearly/{year}/pdf', [LandlordLeaseController::class, 'downloadYearlyPdf'] )->name('lease.yearly.pdf');
        });
